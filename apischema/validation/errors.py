...
@overload
def merge(err1: ValidationError, err2: Optional[ValidationError]
          ) -> ValidationError:
    ...
...
def merge(err1: Optional[ValidationError], err2: Optional[ValidationError]):
    ...
    if err2 is None:
        return err1
    return ValidationError([*err1.messages, *err2.messages], {
...
def _rec_build_error(path: Sequence[str], msg: ErrorMsg) -> ValidationError:
    if not path:
        ...
        return ValidationError(
            children={path[0]: _rec_build_error(path[1:], msg)})
...
def _check_error_path(path: Any) -> Sequence[str]:
    ...
    for i, elt in enumerate(path):
        ...
        if not isinstance(path[i], str):
            raise TypeError(f"Bad error path, expected Field, int or str,"
                            f" found {type(i)}")
    return path
...
def build_from_errors(errors: Iterable[Error]) -> ValidationError:
    ...
    for error in errors:
        ...
        if not path:
            ...
            children[path[0]] = merge(children.get(path[0]),
                                      _rec_build_error(path[1:], msg))
