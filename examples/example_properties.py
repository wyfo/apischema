...
class Serialized:
    ...
    properties: Mapping[str, Any] = field(default_factory=dict, metadata=properties())
...
def test_serialized():
    data = {"class": "Foo", "field1": 0, "field2": ["elt1", "elt2"]}
    serialized = from_data(Serialized, data)
    assert serialized == Serialized(
        "Foo", properties={"field1": 0, "field2": ["elt1", "elt2"]}
    )
