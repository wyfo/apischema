...
def test_tree():
    data = {
        ...
        "branches": [
            {"node": "leaf1"},
            {"node": "node1", "branches": [{"node": "leaf2"}, {"node": "leaf3"}]},
        ],
    }
    ...
    assert tree == Tree(
        ...
        {
            "leaf1": Tree("leaf1"),
            "node1": Tree("node1", {"leaf2": Tree("leaf2"), "leaf3": Tree("leaf3")}),
        },
    )
    ...
    assert to_data(build_input_schema(Tree, ref_factory=ref_factory)) == {
        ...
        "properties": {
            "node": {"type": "string"},
            "branches": {"items": {"$ref": "#/definitions/Tree"}, "type": "array"},
        },
        "required": ["node"],
        "type": "object",
    }
    ...
    data = {
        ...
        "branches": [
            {"node": "leaf1"},
            {
                ...
                "branches": [
                    {"node": "leaf2"},
                    {"node": "node2", "branches": [{"node": "leaf2"}]},
                ],
            },
        ],
    }
            ...
            "branches": ValidationError(
                children={
                    "1": ValidationError(
                        children={
                            "branches": ValidationError(
                                children={"1": ValidationError(["duplicate nodes"])}
                            )
                        }
                    )
                }
